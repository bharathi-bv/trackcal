...
export async function GET() {
  ...
  const { data } = await db
    .from("host_settings")
    .select("host_name, profile_photo_url, booking_base_url, weekly_availability, webhook_urls, calendar_provider, google_calendar_ids, microsoft_calendar_ids, zoom_refresh_token, sheet_refresh_token, sheet_id")
    .limit(1)
    .maybeSingle();
