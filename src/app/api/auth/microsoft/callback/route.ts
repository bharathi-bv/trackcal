...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    const url = isOnboarding
      ...
      : "/app/availability?tab=calendar&calendar_error=access_denied";
  ...
  if (!code) {
    const url = isOnboarding
      ...
      : "/app/availability?tab=calendar&calendar_error=no_code";
    return NextResponse.redirect(new URL(url, request.url));
  }
  ...
  try {
    ...
    const successUrl = isOnboarding
      ? "/app/getting-started"
      : "/app/availability?tab=calendar&calendar_connected=microsoft";
    ...
    const errorUrl = isOnboarding
      ...
      : "/app/availability?tab=calendar&calendar_error=exchange_failed";
